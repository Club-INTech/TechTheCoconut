...
void execute_user_code(void)
{
	...
	//lcd_write ("Running user  app from flash.   ");
 ...
 */
	...
	if (checksum != 0)
	{
		...
		//lcd_write ("Flash contents  failed checksum.");
		return (FALSE);
	}
	else
#endif
...
{
	//if ((boot_buffer[0] != 0xAA) && (boot_buffer[1] != 0xBB) && (boot_buffer[2] != 0xCC) && (boot_buffer[3] != 0xDD))
    if( ( ( LPC_GPIO0->FIOPIN & ( 1L << 25 ) ) ) )
	{
		// Display message to RDB1768 LCD
		//LCD_PrintString2Terminal ("Button not\npressed.\n", LCD_TERMINAL_NoNL,COLOR_BLUE, COLOR_YELLOW);
    //	lcd_write ("Button not      pressed.        ");
		execute_user_code();
	}
	else
	{
